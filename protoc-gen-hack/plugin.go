...
func gen(req *ppb.CodeGeneratorRequest) *ppb.CodeGeneratorResponse {
	...
	for _, fdp := range req.ProtoFile {
		...
		writeFiles(fdp, rootns, genService, allowProto2, resp)
	}
	return resp
}

func writeFiles(fdp *desc.FileDescriptorProto, rootNs *Namespace, genService, allowProto2 bool, resp *ppb.CodeGeneratorResponse) {
	syn := SyntaxUnknown
	...
	for _, edp := range fdp.EnumType {
		writeFile(edp.Name, fdp, rootNs, resp, func(w *writer, _ *Namespace) {
			writeEnum(w, edp, nil)
		})
	}
	...
	for _, dp := range fdp.MessageType {
		writeFile(dp.Name, fdp, rootNs, resp, func(w *writer, ns *Namespace) {
	...
	if genService {
		for _, sdp := range fdp.Service {
			writeFile(sdp.Name, fdp, rootNs, resp, func(w *writer, ns *Namespace) {
	...
	dsn := "Descriptor"
	writeFile(&dsn, fdp, rootNs, resp, func(w *writer, _ *Namespace) {
		w.ln()
...
func writeFile(name *string, fdp *desc.FileDescriptorProto, rootNs *Namespace, resp *ppb.CodeGeneratorResponse, output FileWriter) {
